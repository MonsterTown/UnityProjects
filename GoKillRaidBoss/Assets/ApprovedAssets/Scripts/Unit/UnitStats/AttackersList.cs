...
public class AttackersList : MonoBehaviour {
    ...
    private readonly GameObjectTags selfTags;

    public AttackersList(GameObject self)
    {
        this.self = self;
    ...
    public void AddAttackerToEnemyList(GameObject attacker)
    {
        if (!attackers.Contains(attacker) //Еще нет в листе?
            && attacker.gameObject.transform.root.gameObject != self.transform.root.gameObject /*Не сам*/
            ...
            && attacker.gameObject.GetComponent<GameObjectTags>().fraction != selfTags.fraction)
        {
