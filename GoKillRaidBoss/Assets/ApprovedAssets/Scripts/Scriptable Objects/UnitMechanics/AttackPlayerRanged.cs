...
public class AttackPlayerRanged : AbstractAttack {
    ...
    private void AttackAction(GameObject self) { //Attack With No Target (GroundAttack)
        ...
        rocketClone.velocity = self.transform.forward * speed + self.transform.up * ((secondVector + (0.1f * distance)) * size) * 0.2f + self.transform.right * -0.5f; // Стрела летит вперед*50 + параболическая формула + поправка то что стрела отправляется чуть правее (не с центра модельки)
    }
